...
private:
    struct Select
    {
        ...
        void init(){
            ...
            sel[2] = choose(Search, Vector2 {27,576},Vector2 {150,40},"Search",ConstColor1,ConstColor2,0);
    ...
    public:
        ...
        void dfs(int id,int par,int l,int r,int h, int delta, std::vector<Transforms2> &g,std::vector <TransformsEdge> &f){
            ...
            if (u.left != -1) dfs(u.left,id,l,(l + r)/2,h + delta,delta,g,f);
            if (u.right != - 1) dfs(u.right,id,(l + r)/2,r,h + delta,delta,g,f);
        }
